...
def _projection_poc_content(data: dict[str, Any]) -> None:
    ...
    data["nodes"].update(
        {
            ...
            "projection-adapter": {
                ...
                "contract": "Provider table IDs, headers, links and HTTP behavior stay outside the domain. The adapter structurally matches the application-owned ProjectionSource protocol.",
                "guarantee": "A network failure produces a bounded local error rather than a silent empty snapshot.",
                "limit": "The developer probe instantiates this adapter directly. No application service consumes the projection port yet, and the POC has no browser fallback or authenticated premium session.",
            ...
            "projection-output": {
                ...
                "title": "Ignored local JSON snapshot",
                "layer": "Local POC output",
                "role": "Retains the normalized result on the owner's machine so the acquisition contract can be inspected without publishing provider data.",
                "contract": "A timestamped JSON file is written under .local/projections/hashtag only after acquisition, parsing and validation succeed.",
                "guarantee": "The destination is ignored by Git; the probe prints counts and status rather than player records.",
                "limit": "This is not application persistence, a DuckDB repository or evidence available to the Angular and MCP interfaces.",
                "refs": [refs["projection-probe"]],
            },
        }
    )
    ...
    data["features"].insert(
        0,
        {
            ...
            "focus": "projection-adapter",
            "calls": [
                "Invokes source adapter",
                "Passes HTML",
                "Builds typed facts",
            ],
            "diagram": {
                "nodes": [
                    {"id": "projection-probe", "x": 40, "y": 25, "w": 340, "h": 94},
                    {"id": "projection-output", "x": 475, "y": 25, "w": 285, "h": 105},
                    {"id": "projection-adapter", "x": 40, "y": 225, "w": 340, "h": 94},
                    {"id": "projection-source", "x": 475, "y": 225, "w": 285, "h": 105},
                    {"id": "projection-parser", "x": 40, "y": 425, "w": 340, "h": 94},
                    {"id": "projection-domain", "x": 40, "y": 625, "w": 340, "h": 105},
                ],
                "edges": [
                    {"path": "M210 119V225", "label": "invokes", "x": 230, "y": 176},
                    {"path": "M380 272H475", "label": "HTTP GET", "x": 397, "y": 257},
                    {"path": "M210 319V425", "label": "passes HTML", "x": 230, "y": 376},
                    {"path": "M210 519V625", "label": "builds typed facts", "x": 230, "y": 576},
                    {"path": "M380 72H475", "label": "writes after success", "x": 392, "y": 57},
                ],
            },
            "note": "A developer-only probe directly invokes the public-page adapter, parser and domain validation, then writes an ignored local snapshot. It is not wired into bootstrap, an application service, FastAPI, MCP or Angular.",
        },
    )
